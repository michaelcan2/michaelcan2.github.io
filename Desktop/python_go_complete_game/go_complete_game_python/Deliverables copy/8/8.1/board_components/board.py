...
#########################################
## This module implements the GO board ##
#########################################
...
class Board(BoardInterface):
    def __init__(self, board=None):
        if board:
            ...
            # create blank board if board is not set
            self.repr = [[" " for col in range(BOARD_SIZE_Y)] for row in range(BOARD_SIZE_X)]
    ...
    def __verify_board(self):
        board = self.repr
        # check board length
        assert len(board) == BOARD_SIZE_Y, 'Not a valid board, does not contain BOARD_SIZE_Y rows!'
        for row in board:
            # check row lengths
            assert len(row) == BOARD_SIZE_X, 'Not valid row, it is not length BOARD_SIZE_X!'
        ...
        def reachable_bfs_helper(check_matrix, s, visited, que, result_list):  # s is MaybeStone
            if que.empty():
                ...
                if is_valid_pos(p.x + 1, p.y) and check_matrix[p.x + 1][p.y] == s:
                    result_list.append(Point([p.x + 1, p.y], is_list=True))

                if is_valid_pos(p.x - 1, p.y) and check_matrix[p.x - 1][p.y] == s:
                    result_list.append(Point([p.x-1, p.y], is_list=True))

                if is_valid_pos(p.x, p.y + 1) and check_matrix[p.x][p.y + 1] == s:
                    result_list.append(Point([p.x, p.y + 1], is_list=True))
                ...
                if is_valid_pos(p.x + 1, p.y) and not visited[p.x + 1][p.y] \
                    ...
                    que.put(Point([p.x+1, p.y], is_list=True))
                ...
                if is_valid_pos(p.x, p.y + 1) and not visited[p.x][p.y+1] \
                    ...
                    que.put(Point([p.x, p.y+1], is_list=True))
