...
from board_components.definitions import MIN_HIST_LENGTH, MAX_HIST_LENGTH, MAX_SCORE, WHITE_STONE, \
    BLACK_STONE, EMPTY_STONE, PASS

############################################
## This module implements GO rule checker ##
############################################
...
####################################
## Go Rule Checker Implementation ##
####################################
...
class RuleChecker(RuleCheckerInterface):
    ...
    def move_play(self, s, p, hist):
        recent_board = hist[0]
        # if occupied then play cannot be legal or illegal history
        if recent_board.occupied(p) or not self.verify_history(hist, s):
            return False
        ...
        new_hist = new_hist[:MAX_HIST_LENGTH]

        # validify new hist still follow rules
        if not self.verify_history(new_hist, self.get_opponent(s)):
            return False
    ...
    def get_previous_player_move(self, hist, s):
        if len(hist) > 1:
            prev_board = hist[1]
            next_board = hist[0]
            # get stone counts of {B: int, W: int}
            other_player = self.get_opponent(s)
            prev_stone_places = prev_board.get_points(other_player)
            next_stone_places = next_board.get_points(other_player)
            # check if its a pass
            if prev_board == next_board:
                return PASS
            else:
                for pp in next_stone_places:
                    if pp not in prev_stone_places:
                        return pp
        else:
            return False

    def verify_history(self, hist, s):
        # Rule 1 ~ 4 is covered by the Go Components, here we check Rule 5 ~ 8
        if not self.check_rule_5(hist):
            return False

        if not self.check_rule_6_7(hist, s):
            return False
        
        if not self.check_rule_8(hist, s):
            return False
    ...
    def check_rule_6_7(self, hist, s):
        # Rule 6a: Black moves first. The players alternate thereafter.
        if len(hist) == 1:
            if s != BLACK_STONE:
                return False

        # Rule 6b: The players alternate thereafter.
        # Rule 7: On their turn, a player may either pass or play.
    ...
    def check_rule_8(self, hist, s):
        # Rule 8. A play is illegal if it would have the effect of creating a position that has occurred previously in the game.
