...
class StrategyInterface(with_metaclass(ContractsMeta, object)):
    @abstractmethod
    @contract(stone='str', hist='list[>=$MIN_HIST_LENGTH,<=$MAX_HIST_LENGTH]($Board)', returns='str')
    def make_a_move(self, stone, hist):
        """
...
class CaptureStrategy(StrategyInterface):
    ...
    def move_sequence_possible(self, stone, hist, n, other_player, other_player_move):
        rc = RuleChecker()
        ...
        if rc.move_play(other_player, other_player_move, hist):
            ...
            self.make_a_move(stone, hist, n)
            if self.ai_used_flag:
                return True
        # print("return False")
        return False
        
    def make_a_move(self, stone, hist, n=depth):
        rc = RuleChecker()
...
class RandomStrategy(StrategyInterface):
    def make_a_move(self, stone, hist):
        ...
        for i in range(BOARD_SIZE_X):
            for j in range(BOARD_SIZE_Y):
                pos = Point("{}-{}".format(i+1, j+1))
                if recent_board[j][i] == EMPTY_STONE:
                    moves.append(pos.get_point_repr())

        return random.choice(moves)

class PassStrategy(StrategyInterface):
    def make_a_move(self, stone, hist):
        return PASS
